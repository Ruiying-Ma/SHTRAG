...
class ClusteringOracleConfig:
    def __init__(self, store_json, thresh_is_centered=2.0, round_font_size=2, thresh_is_underlined=6.0, thresh_is_line=2.0, thresh_rect=0.0):
        ...
        Args:
            ...
            - store_json: if not store, set None; else, set as the absolute output path
        '''
...
class ClusteringOracle:
    '''
    Take a pdf and document-layout-analysis result (a list of object) as inputs. Add features to the objects. Cluster the objects according to features.
    '''
    ...
    def cluster(self, pdf_path: str, object_dicts_list: List[Dict]) -> List[Dict]:
        '''
        Extract the features of an object extracted by pdf-document-layout-analysis. Add them to object["features"]. Note that when extracting features, the texts of Section headers/titles are reset as extracted from the pdf file. Return the new object dicts list.

        The template of an object in object_dicts_list must be:
        {
            "left": float,
            "top": float,
            "width": float,
        ...
        Args:
            ...
            - object_dicts_list (List[Dict]): the result of pdf-document-layout-analysis

        Returns

            - new_object_dicts_list (List[Dict]): filter all empty-text objects, add features and cluster_id to Section header/Title/List item
        '''
