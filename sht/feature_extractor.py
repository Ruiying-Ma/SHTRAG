...
class FeatureExtractor:
    '''
    Take a pdf and a document-layout-analysis result object as inputs. Extract features for each result object. 
    '''
    ...
    def extract_font_info(self) -> Dict:
        ...
        from self.rect. For rect info, see https://pymupdf.readthedocs.io/en/latest/textpage.html#textpagedict
    ...
    def extract_clean_text(self) -> str:
        '''
        Extract text from rect and clean the space
        '''
        if self.pdf_doc is None:
            raise ValueError("Please load pdf_doc before extracting font info")
        elif not isinstance(self.pdf_doc, fitz.Document):
