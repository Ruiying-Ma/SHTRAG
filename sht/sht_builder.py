...
class SHTBuilderConfig:
    def __init__(self, store_json: str, load_json: str, chunk_size: int, summary_len: int, embedding_model_name: str, openai_key_path: str="/home/ruiying/Documents/Codebase/config/openai/config_openai.txt"):
        if isinstance(store_json, str):
            ...
            raise ValueError("store_json must be a valid string: the built SHT should be stored in files")

        if load_json is not None:
            if not load_json.endswith(".json"):
                raise ValueError("load_json must be a path to a json file that ends with .json")
            elif not os.path.exists(os.path.dirname(load_json)):
                raise ValueError("load_json directory doesn't exist")
        ...
        if (not isinstance(embedding_model_name, str)) or (not embedding_model_name in ["sbert", "dpr", "te3small"]):
            raise ValueError("embedding_model_name must be 'sbert', 'dpr', or 'te3small'")
...
class SHTBuilder:
    ...
    def _create_node(self, id: int, type: str, parent: int, heading: str, texts: List[str], info: Dict) -> Dict:
        '''
        Create a tree node. Initially, children and embeddings are not created. Children will be created after the parent relations are determined. Embeddings will be created after the whole SHT are created. Also, initially, texts for "head" and "item" nodes are [""]. They will be created after the structure of the SHT are returned. A tree node has:

        - is_dummy (bool): false

        - id (int): 0-based

        - type (str): can be "head", "list", "text" 

        - parent (int): id of its parent. If no parent, -1.

        - children (List[int]): list of ids of its children. In increasing order of id.
        ...
        - heading (str): 
            - "head": heading text
            - "list": first subsentence of list text
            - "text": empty
        ...
        - embeddings (List): 
            - "heading": an embedding for heading if exists (only "head", "list" will have)
            - "texts": [embed(text) for text in texts]
            - "hybrid": [embed(ancestors, text) for text in texts]

        - info (Dict): object dict returned by pdf-document-layout-analysis (only "head", "list" will have)
        '''
        ...
        if not isinstance(heading, str):
            ...
            if type != "text":
                raise ValueError("only 'text' nodes can have empty heading")
        else:
            if type == "text":
                raise ValueError("'text' nodes must have empty heading")
        ...
        if not isinstance(texts, List):
            ...
            if type != "text" and len(texts) > 1:
                raise ValueError("head/list nodes cannot have more than 1 text in texts")
        else:
            if type == "text":
                raise ValueError("'text' nodes' texts must be non-empty")
        ...
        if type == "head" or type == "list":
            if info is None:
                raise ValueError("'head' and 'list' nodes' info cannot be None")
            ...
            assert type == "text"
            if info is not None:
                raise ValueError("'text' nodes' info must be None")
            node["embeddings"] = {
                "texts": None,
                "hybrid": None,
            }
    ...
    def _create_dummy(self, id: int, parent: int) -> Dict:
        '''
        Create a dummy node. Initially children are not given. Children will be created after parent relations are determined.
    ...
    def _place_node(self, pre_node: Dict, rpath: List[int], m_cluster_to_height: Dict, nodes: List) -> int:
        '''
        Create the node from the pre_node. Put it to the right place. Return the node's id. Note that the paren's children is also updated.
        '''
    ...
    def build(self, object_dicts_list: List[Dict]):
        ...
        Args:
            ...
            - object_dicts_list (List[Dict]): the list returned by ClusteringOracle

        Return: the returned tree is stored in self.tree

            - sht (Dict):
                - nodes (List[Dict]): a list of nodes in preorder
                - m_height_to_ids_list (Dict): a map from height to a list of node ids (0-based, increasing order). Contain -1, which is the height of the fake root.
                - m_id_to_height (Dict): a map from node id to height. Contain -1, which is the id of the fake root.
                - full_text (str): join the objects by two 'line_breakers'
    ...
    def _estimate_cost(self):
        '''
        Estimate the LLM part building cost of an sht. Each non-dummy output its heading + texts once. For input_tokens, take the sum of nodes with parent. For input_tokens, take the sum of nodes with children. 
    ...
    def add_summaries(self) -> Dict:
        '''
        The initial tree generated by self.build() doesn't contain summary. Generate summaries for "head" and "list". The texts are concatenated by 2 linebreakers.
        ...
        raise ValueError("Executions error: cannot be executed to this line")
    ...
    def add_embeddings(self, node_ids_list: List[int]) -> float:
        '''
        After building the tree and adding summaries, add embeddings to given tree node ids. 
    ...
    def visualize(self):
        ...
        with open(self.store_json.replace(".json", ".txt"), 'w') as file:
            vis_tree_str = ""
            for pre, _, node in RenderTree(root):
