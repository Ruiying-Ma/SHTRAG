...
class SHTIndexerConfig:
    def __init__(self, use_hierarchy: bool, distance_metric: str, query_embedding_model_name: str, openai_key_path: str=os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), ".env")):
        '''
        Args:
            - use_hierarchy (bool): whether use hybrid-/texts-embeddings
...
class SHTIndexer:
    ...
    def index(self, query: str, nodes: List[Dict]) -> List[Dict]:
        ...
        ```
        {
            "node_id": int, # the id of the node this chunk belongs to
            "chunk_id": int, # the id of this chunk among all its node's chunks
        }
        '''
